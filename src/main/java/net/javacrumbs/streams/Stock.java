...
public class Stock {
    ...
    private void doRun3(List<String> symbols) {
        //can use directly executorService but would have to catch the exceptions on get
        ExecutorService executorService = Executors.newFixedThreadPool(20);

        measure(() ->
                symbols.stream()
                        .map(s -> CompletableFuture.supplyAsync(() -> getStockInfo(s), executorService))
                        .collect(Collectors.toList()).stream()
                        .map(f -> f.join()) //no timeout set WRONG
                        .filter(s -> s.getPrice() < 500)
                        .max(Comparator.comparingDouble(StockInfo::getPrice))
                        .ifPresent(System.out::println)
        );
